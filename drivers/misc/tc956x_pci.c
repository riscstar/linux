 ...
 * an internal endpoint as well on a "virtual" port, which implements
 * two Synopsys XGMAC Ethernet interface.
 *
 ...
 * cases used by) both Ethernet XGMACs.  Each Ethernet MAC must have
 ...
 * the the auxiliary device's platform data with the stmmac driver that
...
/* PCI BAR assignments */
#define PCI_BAR_BRIDGE_CONFIG		0	/* For address translation */
...
/*
 * There are four AXI translation table entries each with eight 4-byte
 * registers. The Ethernet MAC accesses PCI resources through addressses
 * based at TC956X_SLV00_SRC_ADDR, and the first translation table
 * converts those to PCIe address space starting based at 0x0.  We don't
 * use the other three available translation tables.
 */
 ...
 * range, providing specific addresses (and a regmap) needed by the
 * stmmac driver.
 */
...
#define DWMAC_PMA_CTL_OFFSET		0x4000
...
/* We use indirect addressing, so 8 bit range is enough */
static struct regmap_config xpcs_regmap_config = {
	.reg_bits	= 32,
	.val_bits	= 32,
...
{
	u8 mac_id = PCI_FUNC(pdev->devfn) ? 1 : 0;
	void __iomem *sfr = chip->sfr[mac_id];
	...
	dwmac = sfr + DWMAC_OFFSET(mac_id);
	xpcs = dwmac + DWMAC_XPCS_OFFSET;
	xpcs_regmap = devm_regmap_init_mmio(dev, xpcs, &xpcs_regmap_config);
	...
	data->pma_ctl = dwmac + DWMAC_PMA_CTL_OFFSET;
	data->xpcs_regmap = xpcs_regmap;
	data->xpcs_indirect = true;
...
static void chip_translation_config(struct tc956x_chip *chip)
{
	...
	/*
	 * We only use the first AXI4 slave translation table:
	...
	/* This TRSL_PARAM value is assigned for all four translation tables */
	trsl_param_val = u32_encode_bits(TRSL_ID_PCIE_TX_RX, TRSL_ID_MASK);
	trsl_param_val |= u32_encode_bits(TRSL_PARAM_VAL, TRSL_PARAM_MASK);
...
static int tc956x_chip_resume_noirq(struct device *dev)
{
	...
	/* Reconfigure translation tables following suspend */
	chip_translation_config(chip);
