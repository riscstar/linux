				    ...
				    struct tc956x_qcom_priv *qpriv)
{
	...
	if(!qpriv->has_always_on_supplies) {
		u32 phy_reset_gpios[3];
		if (of_property_read_u32_array(dev->of_node, "phy-reset-gpios",
					       phy_reset_gpios, 3) == 0) {
			qpriv->phy_rst_gpio = phy_reset_gpios[1];
	...
	if (of_property_read_u32(dev->of_node, "qcom,phy-rst-delay-us", &qpriv->phy_rst_delay_us)) {
		...
		} else if (of_property_read_u32(dev->of_node, "phy-reset-post-delay", &qpriv->phy_rst_delay_us) == 0) {
			qpriv->phy_rst_delay_us *= 1000;
		} else {
