...
/* If seen when read, this value means "masked"; writing it unmasks it */
...
/* All PMA registers are relative to the pma_ctl address */
...
static struct irq_domain *tc956x_msigen_irq_domain_instantiate(
...
static void tc956x_pma_init(struct tc956x_data *td)
{
	...
	void __iomem *pma_base = td->auxbus_data->pma_ctl;
	...
	for (i = 0; i < PMA_REG_COUNT; i++) {
		...
		writel(0, pma_base + PMA_HWT_REFCK_R_EN + offset);
		writel(0, pma_base + PMA_HWT_REFCK_TERM_EN + offset);

		/* Switch clock from C0_REFCK to CLK_REF_I */
		offset =  i * PMA_COMM_CFG_0_1_STRIDE;
		writel(val, pma_base + PMA_COMM_CFG_0_1 + offset);
	}
...
static int tc956x_pcs_init(struct stmmac_priv *priv)
{
	...
	xpcs_regmap_cfg.regmap = td->auxbus_data->xpcs_regmap;
	xpcs_regmap_cfg.reg_indir = td->auxbus_data->xpcs_indirect;
...
static int tc956x_plat_dat_init(struct tc956x_data *td)
{
	...
	/*
	 * Our MDIO clock rate is fixed at 125 MHz.  For XGMAC, clk_csr 0
