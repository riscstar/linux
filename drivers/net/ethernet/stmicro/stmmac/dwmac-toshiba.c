...
static void tc956x_xpcs_ctrl_ane(struct stmmac_priv *priv, bool ane)
{
	u32 reg_value;
	...
	if (priv->wol_config_enabled != true) {
		...
		if (ane) {
			...
			pr_debug("%s Enable AN", __func__);
		} else {
			reg_value &= (~XGMAC_AN_37_ENABLE);
			pr_debug("%s Disable AN", __func__);
			}
#ifdef CONFIG_TC956X_MAGIC_PACKET_WOL_CONF
	} else {
 ...
 *  @remarks : -
 */
static void tc956x_xpcs_ctrl0_lrx(struct stmmac_priv *priv, bool lrx)
{
	u32 reg_value;
	...
	if (lrx) {
		...
		pr_debug("%s Enable XPCS LPI Rx\n", __func__);
	} else {
		reg_value &= (~XGMAC_EEE_LRX_EN);
		pr_debug("%s Disable XPCS LPI Rx\n", __func__);
	}
...
int tc956x_logstat_state_log_analyze(uint32_t cur_state)
{
	...
	if (timeout == LTSSM_TIMEOUT_OCCURRED)
		pr_info("==> LTSSM Timeout occurred!\n");
	else {
		...
		if (LinkSpeed_Pre != LOGSTAT_DUMMY_VALUE) {
			if (LinkSpeed < LinkSpeed_Pre)
				pr_debug("==> Speed down occurred! (Gen%d --> Gen%d)\n", LinkSpeed_Pre, LinkSpeed);
			else if (LinkSpeed > LinkSpeed_Pre)
				pr_debug("==> Speed up occurred! (Gen%d --> Gen%d)\n", LinkSpeed_Pre, LinkSpeed);
		}
		...
		if ((LinkWidth_Pre != LOGSTAT_DUMMY_VALUE) && (LinkWidth_Pre != ALL_LANES_INACTIVE)) {
			if (LinkWidth < LinkWidth_Pre) {
				if (strcmp(ltssm_dec, "Detect.Active") == 0) {
					if (LinkWidth > 1)
						pr_debug("==> Receiver Detection is occurred! (Only %d Lanes is detected)\n", LinkWidth);
					else
						pr_debug("==> Receiver Detection is occurred! (Only 1 Lane is detected)\n");
				} else
					pr_debug("==> Link Width down configure occurred! (x%d --> x%d)\n", LinkWidth_Pre, LinkWidth);
			} else if (LinkWidth > LinkWidth_Pre)
				pr_debug("==> Link Width upconfigure occurred! (x%d --> x%d)\n", LinkWidth_Pre, LinkWidth);
		}
...
static int tc956x_set_pci_speed(struct pci_dev *pdev, u32 speed)
{
	struct pci_dev *usp;
	struct pci_dev *root;
